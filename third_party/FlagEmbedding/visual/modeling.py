...
class Visualized_BGE(nn.Module):
    def __init__(self,
        ...
        assert model_name_bge in ["BAAI/bge-document-en-v1.5", "BAAI/bge-m3"]
        assert model_weight is not None

        if model_config_location == None:
            model_config_location = model_name_bge
        self.model_name_bge = model_name_bge
        
        if model_name_bge == 'BAAI/bge-document-en-v1.5':
        ...
        bge_config = AutoConfig.from_pretrained(model_config_location)
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(model_config_location,use_fast=False)
    ...
    def encode(self, image=None, text=None, device=torch.device('cpu')):        # device是原版没有的
        # used for simple inference
        if image is not None:
            image = self.preprocess_val(Image.open(image)).unsqueeze(0).to(device)
            if text is not None:
                text = self.tokenizer(text, return_tensors="pt", padding=True).to(device)
